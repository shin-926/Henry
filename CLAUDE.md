...
1. **動作確認**（chrome-devtools-mcp）- メイン
   - ページをリロード（または該当画面に遷移）
   - `list_console_messages` でエラー確認
   - **修正内容に応じた操作**を実行（click, fill等）
   - 期待通りの動作を `snapshot` または `screenshot` で確認
   - 必要に応じて `list_network_requests` でAPI呼び出し確認

2. **リグレッションテスト**（Playwrightテストがあれば）
   - `npx playwright test tests/<対象>.spec.js`
   - 基本機能が壊れていないかの確認
   - 失敗したら原因を調査し修正
   - テスト対応表は `NOTES.md` を参照

3. **エラー発生時は自ら解析**
   - `list_console_messages` でエラー内容を確認
   - `list_network_requests` → `get_network_request` でAPI エラーの詳細を取得
   - `take_snapshot` でDOM状態を確認
   - 解析結果に基づいて修正→再確認
...
| **カルテ** | henry_hospitalization_data.user.js | 入院データ表示（実験） |
...
（現在は空。新しいルールが追加されたらここに記載し、後で適切なセクションに移動する）
