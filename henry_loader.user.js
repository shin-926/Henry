...
(function() {
  ...
  // NOTE: 本番用と開発用(henry_loader_dev.user.js)でコードの約60%が重複しているが、
  // 統合は検討済みで見送り（Issue #40, 2026-02）。理由:
  // - Loaderの変更頻度が極めて低く、同期の手間が実質発生していない
  // - 差分は設定値（URL/ブランチ/ベータ版フィルタ/実行方式）が中心で、
  //   統合すると本番/開発の分岐ロジックが増えてかえって読みにくくなる
  // - 2ファイル×300行程度の規模では、統合の保守性メリットが小さい
