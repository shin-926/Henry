...
function convertCheckboxValue(value, mapping) {
  ...
  const checkboxOptions = {
    '{{他科名}}': ['内科', '精神科', '外科', '整形外科', '脳神経外科', '皮膚科', '泌尿器科', '眼科', '耳鼻咽喉科', '歯科', 'リハビリ科', '放射線科', 'その他'],
    '{{処置内容}}': ['点滴の管理', '中心静脈栄養', '透析', 'ストーマの処置', '酸素療法', 'レスピレーター', '気管切開の処置', '疼痛の看護', '経管栄養'],
    '{{特別な対応}}': ['モニター測定', '褥瘡の処置'],
    '{{失禁への対応}}': ['カテーテル'],
    '{{周辺症状詳細}}': ['幻視･幻聴', '妄想', '昼夜逆転', '暴言', '暴行', '介護への抵抗', '徘徊', '火の不始末', '不潔行為', '異食行動', '性的問題行動', 'その他'],
    '{{発生可能性状態}}': ['尿失禁', '転倒・骨折', '痛み', '褥瘡', '徘徊', 'うつ状態', '意欲低下', '閉じこもり', 'リハビリテーションの必要性', '嚥下障害', '口腔衛生管理の必要性', '栄養管理の必要性', '服薬管理の必要性', 'その他'],
    '{{医学的管理の必要性}}': ['訪問診療', '訪問看護', '看護職員の訪問による相談・支援', '訪問歯科診療', '訪問薬剤管理指導', '訪問リハビリテーション', '短期入所療養介護', '訪問歯科衛生指導', '訪問栄養食事指導', '通所リハビリテーション', 'その他'],
  ...
  const options = checkboxOptions[placeholder];
  ...
  // ビットフラグを解析して□/■形式で出力
  const bitString = String(value).padStart(options.length, '0');
  const result = [];

  for (let i = 0; i < options.length; i++) {
    const mark = bitString[i] === '1' ? '■' : '□';
    result.push(mark + options[i]);
  }
