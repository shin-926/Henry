 ...
 *
 * ■ 共通化の範囲について（Issue #40 で調査済み・2026-02）
 * このモジュールで共通化できる部分（Google API、GraphQL、モーダル骨格、下書き管理、
 * Docs出力フロー等）は既に集約済み（共通化率 70-80%）。
 * 各スクリプトに残っている以下の部分は病院ごとに異なるため、さらなる共通化は見送り：
 * - buildFormBody(): 病院固有の入力項目・レイアウトが多様
 * - setupFormEvents(): 条件付き表示ロジック（受診歴、検査種別等）が病院ごとに異なる
 * - collectFormData(): 病院固有フィールドの収集
 * - 日付フォーマット: 7パターン以上（元号略記、AM/PM、曜日等の組み合わせ）
 * テンプレート化すると設定オブジェクトが肥大化し、かえって複雑になるため現状維持が妥当。
 *
 * ■ 公開API
